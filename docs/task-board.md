...
| DEV-004 | Phase 1 | External Repo Structure Analysis | Explorer | Done | Validated | P0 | 100 | /home/u24/projects/AStore | AStore 구조 분석 결과 | module별 adapter 규칙 심화 | 세부 XML/action 규칙은 후속 |
...
| DEV-023 | Phase 8 | Adapter Framework | Main / Trace Worker | Done | Validated | P1 | 100 | docs/spring-plugin-architecture.md | adapter selection + astore-legacy adapter | adapter merge/canonicalization 고도화 | alias 중복 모델링은 후속 정리 |
| DEV-024 | Phase 8 | Module Classification / Adapter Recommendation | Main | Done | Validated | P1 | 100 | /home/u24/projects/AStore | module classifier + adapter recommendation API | module별 adapter 고도화 | 세부 adapter 내부 규칙은 후속 심화 |
...
- adapter merge/canonicalization 유지 및 추가 adapter pack 확장
- AStore-Seller / Admin / Carrier / batch / lib 별 세부 규칙 강화
- XML handler mapping, batch shell/job graph, shared provider 외부 호출 규칙 반영
- 실제 브라우저 수집 SDK 우선순위 재정렬
